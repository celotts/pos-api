...
const TaxForm: React.FC<TaxFormProps> = ({ onSubmit, onCancel, initialData }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input id="name" name="name" value={formData.name} onChange={handleChange} required className={inputStyle} />
      </div>
      <div>
        <label htmlFor="percentage" className={labelStyle}>Percentage (e.g., 0.16 for 16%)</label>
        <input id="percentage" type="number" name="percentage" value={formData.percentage} onChange={handleChange} required step="0.0001" className={inputStyle} />
      </div>
      <div>
        <label htmlFor="taxType" className={labelStyle}>Tax Type</label>
        <select id="taxType" name="taxType" value={formData.taxType} onChange={handleChange} className={inputStyle}>
